        ...
        <div className="container-lg">
          ...
          <div className="max-w-[66.8125rem] p-[3.125rem] rounded-[1.25rem] bg-white shadow-[0_5px_27px_0_rgba(57,69,85,0.09)] mx-auto mt-16 relative min-h-[54.3125rem]">
            <div className="grid grid-cols-3 gap-[1.625rem] absolute w-full -top-[0.875rem] right-[1.6875rem] max-w-[49.125rem]">
              {getdata?.data?.basic_details?.map((item, i) => (
                <div
                  className="c-card c-card--membership-subscription c-card--membership-subscription-active p-6 
                 rounded-xl flex items-center justify-center flex-col 
                 transition group min-h-[24.5625rem]"
                  key={i}
                  style={{
                    backgroundImage: `${
                      i == 1 ? `url(${PlanHoverBg.src})` : null
                    }`,
                    backgroundSize: `${i == 1 ? 'cover' : null}`,
                    position: `${i === 1 ? 'relative' : null}`,
                  }}
                >
                  <div className="flex-1 text-center">
                    <h1
                      className={`text-xl font-normal ${
                        i === 1 ? 'text-white' : 'group-hover:text-white'
                      }  mt-5`}
                    >
                      {item?.name}
                    </h1>
                    <h2
                      className={`text-[2.5rem] font-bold py-6 ${
                        i === 1 ? 'text-white' : 'group-hover:text-white'
                      }`}
                    >
                      ${item?.monthly_price}
                      <span
                        className={`text-[0.9375rem] font-normal text-[#686E80] ${
                          i === 1 ? 'text-white' : 'group-hover:text-white'
                        }`}
                      >
                        /month
                  ...
                  <div
                    className={`divider after:w-[0.0625rem] my-0 group-hover:before:bg-white group-hover:after:bg-white ${
                      i === 1
                  ...
                  {i === 1 && (
                    <div className="w-[7.25rem] overflow-hidden absolute top-2 right-0">
                      <Image
                        src={MostPopularRibbonBadge}
                        alt="Most popular ribbon badge"
