...
const RateAndPaymentTerms = ({ userid }) => {
    ...
    const { data: getdata, isLoading, mutate } = useSWR([`/profile/getPaymentFee/${userid}`, axiosAuth], ([url, axiosAuth]) => WithTokenGetApi(url, axiosAuth), {
    ...
    const schema = z.object({
        ...
        days_before_call: z
            .string()
            .min(1, { message: "Select payment terms" }),
    ...
    const onSubmit = (data) => {
        let modifyData = { ...data, "user_id": userid, download_fee: null };
            ...
            <form onSubmit={handleSubmit(onSubmit)}
                ...
                <div className="c-case-wrap-col w-full pt-4 pb-2">
                    ...
                    <div className="form-field textarea">
                        <textarea name="" id="" className="form-control" {...register('refund_policy')}></textarea>
                    </div>
                    <p className="text-red-700 mb-0">
                ...
                <button type="submit" disabled={!isDirty}
                    className={`primary-btn mt-[1.875rem] ${!isDirty || isMutating ? 'disable' : ''}`}>
                    Update
                </button>
            </form>
        </>
