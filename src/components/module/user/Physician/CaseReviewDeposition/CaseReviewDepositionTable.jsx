...
import CaseReviewSearch from './CaseReviewSearch';
import { useSession } from 'next-auth/react';
import moment from 'moment';
import CaseReviewDetailsModal from '../Modal/CaseReviewDetailsModal';
import Loader from '../../lawfirm/Loader';

const CaseReviewDepositionTable = () => {
  ...
  return (
    <>
      <div className="p-title-flex flex items-center justify-between flex-wrap mb-6">
        <h2 className="c-page-title">Case Review Deposition</h2>
        <Link
          href={`${session?.user?.role_id === "8" ? "/Expert" : "/Physician"}/caserequest/createcaserequest`}
          //  href="/Physician/caserequest/createcaserequest"
          htmlFor="create-user"
          className="primary-btn cursor-pointer"
        >
          <Image src={addicon} alt="" /> Add New Case Review
        </Link>
      </div>
                                ...
                                <div className="items-center !p-0">
                                  ...
                                  <div className="tooltip text-left" data-tip="View">
                                    <button className="w-9 overflow-hidden flex items-center justify-center p-2 flex-[0_0_0.5625rem] cursor-pointer">
                                      <label htmlFor='reviewmodal' onClick={() =>
                                        handlemodal(
                                          item?.id,
                                          item?.status
                                        )
...
export default CaseReviewDepositionTable;