...
const EditTimeModal = ({
  ...
  const [depopayload, setdepopayload] = useState('');
  const format = 'H:mm';
  ...
  const { trigger: Timeupdatemailtrigger, data: timeupdatemaildata } =
    useSWRMutation(
      `/case/sendMailAfterTimeUpdate/${depoid}`,
      WithTokenWithoutTriggerGetApi
    );
  useEffect(() => {
    if (posttimedata === undefined && posttimeerror?.response?.status === 300) {
      setstartdateerror('Sorry this time is not available');
    }
    if (posttimedata?.status === 200 || posttimedata?.status === 202) {
      posteventtrigger({ payload: depopayload, axios: axiosAuth });
    }
  }, [posttimedata, posttimeerror]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (isMutating) {
      Timeupdatemailtrigger({ axios: axiosAuth });
      toast.success('Deposition edit successfully');
  ...
  useEffect(() => {
    if (getdepodata?.status === 200) {
      ...
      if (depoid !== clickdepoid) {
        ...
        setendtime(endvalue);
        setuserdata(
          depoid == clickdepoid ? formdata : getdepodata?.data?.invitee_list
        );
      }
    }
  }, [getdepodata]);
  ...
  const handleSave = () => {
    ...
    if (!isError) {
      ...
      let filterinvitee = userdata?.filter((item) => item?.user_id !== null);
      let payload = {
      ...
      let depopayload = {
        ...
        type: other?.type,
        single_call_history_id: other?.single_call_history_id,
        audio: other?.audio,
        invite_users: inviteeuser,
      };
     setdepopayload(depopayload);
     posttimetrigger({ payload, axios: axiosAuth });
    }
  };
              ...
              <div className="flex gap-5 pt-3">
                <div className="flex items-start gap-2 relative flex-[0_0_15.3125rem]">
                  ...
                  <div className="flex-1">
                    ...
                    <TimePicker
                      ...
                      disabled={
                        startcalendarvalue === '' || depoid != clickdepoid
                      }
                ...
                <div className="relative flex-[0_0_6.9375rem]">
                  ...
                  <TimePicker
                    ...
                    value={endtime}
                    className="s-time-picker"
                    disabled={endcalendarvalue === '' || depoid != clickdepoid}
                  ...
                  {enderror !== '' && (
                    <div className="cl-col basis-auto">
                      <span className=" text-red-700 text-xs">{enderror}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <li className="flex items-start py-[0.625rem]">
            ...
            <div className="flex-1">
              <h2 className="text-[0.875rem] font-normal">
                {userdata?.length} guests
              </h2>
              <ul className="-my-2 pt-3">
                {userdata?.map((invitee, i) => (
