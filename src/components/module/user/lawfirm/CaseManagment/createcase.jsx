...
const Createcase = () => {
  ...
  } = useSWRMutation('/fileUpload/case_upload', WithTokenMultipleFormdataPostApi);

  const {
    trigger: posttrigger,
    data: postdata,
    error: posterror,
  } = useSWRMutation(`/case/addCase`, WithTokenPostApi);
  useEffect(() => {
    if (filedata?.status === 200) {
      delete payload['files'];
      let temppayload = { ...payload, files: filedata?.data?.fileData };
      posttrigger({ payload: temppayload, axios: axiosAuth });
    }
  }, [filedata]);
  ...
  useEffect(() => {
    if (getcountydata?.status === 200) {
       if (countydata?.length === 0 && getcountydata?.data?.data?.count?.length !== 0) {
  ...
  useEffect(() => {
    ...
    if (postdata?.status === 200) {
      ...
      redirect(`${data?.user?.role_id === "6" ? "/AttorneyAssistant" : "/Attorney"}/casemanagement`);
    }
  }, [postdata, posterror]);
  ...
  const onSubmit = (data) => {
    ...
    if (file?.length > 0) {
      ...
      posttrigger({ payload: finalpayload, axios: axiosAuth });
    }
  };
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <div className="c-case-wrap-row pt-8">
              ...
              <div className="grid grid-cols-7 gap-3 mt-4">
                {file?.length > 0 &&
                  file?.map((item, i) => (
                    <div className="bg-[#e0eefb] border border-solid border-[#8790af] p-3 rounded-3xl text-[#125ecb] h-12 text-sm font-medium flex items-center justify-between leading-normal gap-3">
                      <span>{item?.name}</span>
                      <div
