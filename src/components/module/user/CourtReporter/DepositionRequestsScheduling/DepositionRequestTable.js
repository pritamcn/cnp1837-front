...
import NoVideoIcon from '../../../../../assets/images/deposition-requests-and-scheduling/no-video-icon.svg';
import ChatIcon from '../../../../../assets/images/deposition-requests-and-scheduling/chat-icon.svg';
import UploadIcon from '../../../../../assets/images/deposition-request/upload-icon.svg';
import MoreVeticalIcon from '../../../../../assets/images/chat/more-vertical-icon.svg';
import VideoDownloadIcon from '../../../../../assets/images/deposition-request/av-download-icon.svg';
...
const DepositionRequestTable = () => {
  ...
  const [uploadDepoId, setUploadDepoId] = useState('');
  ...
  const {
    ...
    [
      `/case/getDepoCallListForInvitee?sort_name=${sortName}&sort_order=${asc}&size=${endpagecount}&page=${startpagecount}&start_date=${startdatevalue}&end_date=${enddatevalue}&case_name=${searchCaseValue}&deposition_number=${searchDepoValue}&status=${status}`,
      axiosAuth,
    ],
    ...
    if (
      ...
      let modifyData = depositionData?.data?.data.map((item) => {
        return {
          ...
          call_download_url: item?.call_download_url,
          isShow: false,
        };
      });
  ...
  return (
    <>
                      ...
                      depositionList.map((item, index) => {
                        return (
                          <tr key={index}>
                                    ...
                                    <div className="items-center !p-0">
                                      ...
                                      {![0, 3].includes(
                                        parseInt(item?.depo_status)
                                          ...
                                          >
                                            <Link
                                              href={`/CourtReporter/depositionmanagement/chat/${item?.id}`}
                                              className="w-9 overflow-hidden flex items-center justify-center p-2 flex-[0_0_0.5625rem]"
                                            >
                                              <Image
                                                src={ChatIcon}
                                      ...
                                      {item?.depo_status === 3 && (
                                        <div
                                          className="tooltip tooltip-right"
                                          data-tip={`${item?.call_download_url
                                            ? 'Download Record'
                            ...
                            <td>{typeof item?.case_name == 'string' ? item?.case_name : <>{item?.case_name[0]}
      ...
      {uploadDepoId && (
        <>
          <UploadFileModal depo_id={uploadDepoId} handleclose={handleclose} />
        </>
      )}
    </>
  );
