...
const CancellationRefundTable = () => {
    ...
    const { data: getcasedata, isLoading } = useSWR(
        [
            `/payment/getPaymentList?deponent_name=${searchvalue}&startCount=${startpagecount}&endCount=${endpagecount}&sort_order=${asc}&sort_name=${sortName}&startDate=${startdatevalue}&endDate=${enddatevalue}&status=${status}`,
            axiosAuth,
        ],
